...
def main_times_10(episodes, alpha=0.1, gannma=0.9, epsilon=0.1, edge_len=25, prj_name=None):
    # assertion
    assert episodes >= 100, ('EPISODES ARE DEFINED AS TO BE MORE THAN 99 !!')
    assert prj_name is not None, ('SET PROJECT NAME !!')
    # log
    ...
    # plotting
    steps_mean = np.mean(steps_lists, axis=0)
    plot_graph = utils.make_plot([e for e in range(1, episodes+1)], steps_mean)
    cv2.imwrite(save_path, plot_graph)


if __name__=='__main__':
    main_times_10(episodes=850, alpha=0.005, gannma=0.99, epsilon=0.05, edge_len=10, prj_name='times10_R100_A0005_G099_E00_epi850')
    # main(episodes=101, alpha=0.005, gannma=0.99, epsilon=0.05, edge_len=10, prj_name='test_R100_A0005_G099_E005')